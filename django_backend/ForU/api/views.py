...
class RecentlyHot(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 질의를 하기위한 쿼리문
        query = f'''select 1 id,song_id,thumb_url,artist_name,song_name,genre_big_name,added_cnt,album_id from (select a.song_id,thumb_url,album_id,added_cnt,artist_name,song_name,genre_big_id from (select song_id,sas.thumb_url,album_id,added_cnt,artist_name,song_name from (select s.song_id,thumb_url,album_id,added_cnt,artist_id,song_name from song as s join artist_song as sa on s.song_id = sa.song_id where s.song_id IN (295492,483763, 80343,443160,110943,326388,317579,621033,402113,348476,558474,76402,198232,163285,204800,269723,5760,247927,66681,70306,433666,539804,479035,69008,186154,875,639150,238134,407279,44463,153256,436967) ) as sas join artist as a on sas.artist_id = a.artist_id) as a join genre_big_song as gb on a.song_id = gb.song_id) as A join genre_big as B on A.genre_big_id = B.genre_big_id group by album_id order by added_cnt desc;'''
